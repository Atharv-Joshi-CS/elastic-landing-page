...
const ReportsEventsWebinars = ({data,type} : {data : ReportEventsWebinarsTD, type : string}) => {
    ...
    const rewCardComponents : React.ReactNode[] = Array.from(data.card.map((card : REWCardTD) => !card ? <></> : <ReportsEventsWebinarsCard key={card._metadata.uid} data = {card} type = {type}/>));    
    return (
        <div className={styles.raw_outer_container}>
            <div className={rawContainer}>
                {!data.title || data.title === '' ? <></> :<p className={styles.section_title}>{data.title}</p>}
